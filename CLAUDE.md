...
- `init` seeds facet categories (`people / topics / projects / sessions`) in the `facets` DB table, not as real directories on the backing fs. Older inits created real dirs of the same names under `~/.memfs/mount`, which shadowed FUSE's virtual view when the mount was stale and caused writes to land in unindexed backing files. As of 0.12.5 `init` empties `~/.memfs/mount` outright (`remove_dir_all` + `create_dir_all`) before mounting, so any leftover real dirs — default-named or otherwise — are wiped. The mount path is hardcoded to `~/.memfs/mount` in the init code path; `MEMFS_MOUNT` only affects the daemon-mount subcommands, not init
- `dates` was a default facet through 0.12.4 and was dropped in 0.12.5 because it's rarely used by clients and not worth the seed cost. Existing installs keep their `dates` facet row (db migrations don't drop it) and clients can still create a `dates` facet on demand via `mkdir`/`memfs mkdir`/auto-tag — `create_facet` is `INSERT OR IGNORE`
